...
class PointsRemoteDataSource {
  ...
  Future<Either<Failure, List<Point>>> getPointsByFilter(Filter filter, Coordinates gps, double range) async {
        ...
        if (filter.connector != null) {
          url = url+ 'connector=' + mapConnectorTypesToString(filter.connector!) + '&'; 
        }
  ...
  Future<TravelDistance?> getTravelDistance(Coordinates origin, Coordinates destination) async {
    var url = 'https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${origin.lat},${origin.long}&destinations=${destination.lat},${destination.long}&key=AIzaSyBrqanf0JgOKydUfoWZ-eZqq97Mhr38ePQ';
