...
double getCurrentPriceFromTariffs(List<Tariff> tariffs) {
  ...
  tariffs.forEach((element) {
      ...
      if ((date.isAfter(start, ) && date.isBefore(end) || (date.isAtSameMomentAs(start)) || date.isAtSameMomentAs(end) )) {
        price = element.price;
      } 
  });
...
void setupBlocListener<T, L, E, S>(BuildContext context, T state, void Function(S state) onSuccess) {
    var dialog = DialogBuilder();
    var isVisible = TickerMode.of(context);

    if (state is L) {
      if (ModalRoute.of(context) != null && ModalRoute.of(context)!.isCurrent && isVisible) {
        dialog.showLoadingDialog(context);
      }
    } else if (state is E) {
      if (ModalRoute.of(context) != null && ModalRoute.of(context)!.isActive && isVisible) {
